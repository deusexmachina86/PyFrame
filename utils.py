        ...
        for cur_src in filepaths:
            ...
            try:
                ...
                for cur_sig in signal_names:
                    try:
                        tstarts.append(np.min(cur_measurement['sys']['INCA']['data'][cur_sig]['data']['!ADAF_Basis!'][:]))
                        tends.append(np.max(cur_measurement['sys']['INCA']['data'][cur_sig]['data']['!ADAF_Basis!'][:]))
    ...
    ##                                      Kiryl Bletsko   (kiryl.bletsko@volvocars.com)                                                
                for (cursig, curmethod) in zip(signal_names, interp_strategy):
                    ...
                    if cursig in missing_signals:
                        ...
                        try:
                            f_i = interp1d(
                                cur_measurement['sys']['INCA']['data'][cursig]['data']['!ADAF_Basis!'][:],
                                cur_measurement['sys']['INCA']['data'][cursig]['data'][cursig][:],
